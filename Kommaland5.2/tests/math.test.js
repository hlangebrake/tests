...
test('45 eindeutige Quests, 393 vollständige Aufgaben und 83 Wissensschritte',()=>{
 assert.equal(QUESTS.length,45);assert.equal(new Set(QUESTS.map(q=>q.id)).size,45);
 assert.equal(QUESTS.reduce((n,q)=>n+q.tasks.length,0),393);
