# Kommaland 5.2 · Lernwelt und Lernblick für Lehrkräfte

Statisch bereitstellbare HTML-/CSS-/JavaScript-Anwendungen. Kein Build, keine Anmeldung,
kein Backend, keine Cloud-Datenbank und kein CDN. Der mathematische Aufgabenkern aus Version 5.1 bleibt erhalten: 45 Quests, 393 Aufgaben,
Wegprüfungen, 83 interaktive Wissenskarten und die drei Bauzustände. Version 5.2 ergänzt
transparente Kompetenzziele, Standortchecks, offene Werkstätten und einen gleichwertigen
2D-Lernzugang.
...
3. Alle alten Kommaland-Fenster schließen und die Adresse neu öffnen. Im Rucksack
   steht „Kommaland 5.2“. Vorhandene Namen/IDs bleiben erhalten; ohne Namen zuerst Name/Kürzel eintragen.
   Auch eine separat gehostete Lehrkräfte-Seite auf 5.2 aktualisieren.
...
Austauschformat hat unabhängig davon `schemaVersion: 2` (Format 1 aus Version 5.0 bleibt lesbar).
...
Beim Sitzungsstart einen kurzen freiwilligen Plan wählen. Im Rucksack lassen sich
„Mein Lernplan“ und „Meine Unterstützung“ jederzeit ändern.

Am Ende: **Rucksack → Lerneinheit abschließen**. Es folgen Plan-Ist-Vergleich und zwei
freiwillige Fragen (Sicherheit, nächstes Vorhaben). Erst bei „Abschließen und exportieren“
wird die Sitzung geschlossen und
...
## Neu in 5.2: selbstständiger, kompetenzorientierter Lernweg

- Jede Quest und jeder Lernbereich zeigt ein explizites **Ich-kann-Ziel** und ein Erfolgskriterium.
- Pro Gebiet gibt es einen **Standortcheck** mit fünf vorhandenen, fachlich geprüften Aufgaben. Ab 80 % können reguläre Quests für die Navigation freiwillig übersprungen werden. Dabei entsteht **kein künstlicher Questabschluss** und keine Note.
- Pro Gebiet gibt es eine **offene Mathematikwerkstatt** zum Begründen, Darstellen, Modellieren und kooperativen Arbeiten. Freie Antworten werden bewusst nicht automatisch bewertet oder gespeichert.
- Der **2D-Lernmodus** bietet Lernziele, Standortcheck, Werkstatt und Quests ohne Navigation durch die 3D-Welt. Wegprüfungen gelten dort genauso.
- Die Schüleransicht fasst die fünf internen Unterstützungsdimensionen in zwei verständliche Entscheidungen zusammen: Orientierung im Lernweg und Hilfe beim Feststecken.
- Das Lehrkräfte-Dashboard ergänzt Kennzahlen um **fachdiagnostische Gesprächsfragen** und Beobachtungskriterien. Automatische Daten werden ausdrücklich nur als Gesprächsanlass behandelt.

Die Selbstregulationslogik aus 5.1 bleibt erhalten: sparsame Impulse, freiwillige Gründe,
Sitzungsrückblick und evidenzbasiertes schrittweises Fading. `SELBSTREGULATION.md`
beschreibt Regeln, Aktionen, Grenzen und die Architektur. Die 393 mathematischen
Questaufgaben, Lösungen, Weltmodelle und Wegprüfungsregeln wurden nicht verändert.

## Bestehende Spielstände und wichtige Grenzen

- Lernstände aus Version 5.0 behalten Identität, Geschichte und mathematischen Stand.
  Neue Regulationsereignisse beginnen erst mit der Aktualisierung. Das neue Dashboard
  liest Austauschformat 1 und 2; ein altes 5.0-Dashboard liest neue Format-2-Dateien nicht.
...
- `SELBSTREGULATION.md`: konkrete Unterstützung, freiwillige Angaben und Startheuristiken.
...
- `docs/version-4.1/` und `docs/version-5.0/`: archivierte frühere Release-Dokumente; nicht der aktuelle Teststand.

```sh
npm test
node tools/build-catalog.mjs
node tools/release-check.mjs
node tools/analytics-release-check.mjs
xvfb-run -a python3 tests/browser_regression.py
xvfb-run -a python3 tests/regulation_browser.py
# Ältere Browser-Skripte in tests/analytics sind als frühere Referenz vorhanden.
```
