...
export const defaultState = () => ({
 ...
 mastery:{}, competencies:{}, exams:{}, placement:{}, workshops:{},
 activeQuest:'v0a', tutorial:false, encounters:{count:0,distance:0},
...
export function validateState(raw) {
 ...
 if(raw.schemaVersion===SCHEMA){
  for(const field of ['mastery','competencies','exams','placement','workshops'])if(raw[field]!==undefined&&!plain(raw[field]))throw new Error('Ungültige Testdaten.');
  ...
  for(const [id,v] of Object.entries(raw.placement||{})){
   if(!REGIONS.some(r=>r.id===id)||!plain(v)||typeof v.passed!=='boolean'||!integer(v.score,0,20)||!integer(v.total,1,20)||v.score>v.total||!validDate(v.at))throw new Error('Ungültiger Standortcheck.');
   s.placement[id]={passed:v.passed,score:v.score,total:v.total,at:v.at};
  }
  for(const [id,v] of Object.entries(raw.workshops||{})){
   if(!REGIONS.some(r=>r.id===id)||!plain(v)||v.done!==true||!validDate(v.at))throw new Error('Ungültige Werkstattangabe.');
   s.workshops[id]={done:true,at:v.at};
  }
 ...
 s.activeQuest=questById(raw.activeQuest)?raw.activeQuest:'v0a';
 ...
 if(legacy) {s.activeQuest='v0a';s.migration={from:1,restarted:Object.keys(raw.progress).length};}
