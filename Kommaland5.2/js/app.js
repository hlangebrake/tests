...
import {REGION_GOALS, OPEN_TASKS, placementTasks, questCompetencyGoal, questSuccessCriterion, learningEvidence} from './didactics.js';

const $ = id => document.getElementById(id);
const panel = $('panel'), overlay = $('overlay'), shell = $('gameShell');
let disposeLabs = ()=>{}, disposeWidgets=()=>{};
let mode = '', session = null, placementSession = null, returnFromInfo = null, lastFocus = null;
...
srlUI=new RegulationUI(regulation,{
 ...
 regions:REGIONS,quests:QUESTS,unlocked:q=>questUnlocked(q,evidence()),satisfied:q=>questSatisfied(q),recommended:recommendedQuest,walk:walkToQuest,
...
function evidence(){return learningEvidence(state,QUESTS);}
function placementPassed(region){return !!state.placement?.[region]?.passed;}
function questSatisfied(q){return !!state.completed[q.id]||(!q.challenge&&placementPassed(q.region));}
function enterLearning(){regulation.sync();updateHUD();if(regulation.needsPlan())srlUI.planning();else closeToWorld();}
function recommendedQuest(preferred=world.region().id){const ev=evidence(),choices=QUESTS.filter(q=>!q.challenge&&!questSatisfied(q)&&questUnlocked(q,ev));return choices.find(q=>q.region===preferred)||choices[0]||QUESTS.find(q=>q.challenge&&questUnlocked(q,ev)&&!state.completed[q.id]);}
function strategyAction(strategy){
 ...
 if(strategy==='drawing'){returnFromInfo=()=>showTask();showKnowledge(t.lesson,false);return;}
...
function chooseNext(preferred) {
 const ev=evidence(),choices=QUESTS.filter(q=>!questSatisfied(q)&&questUnlocked(q,ev));
...
function showQuestIntro(q) {
 ...
 const done=state.completed[q.id],progress=state.progress[q.id],r=regionById(q.region),placed=!q.challenge&&!done&&placementPassed(q.region);
 const goal=questCompetencyGoal(q),criterion=questSuccessCriterion(q);
 openPanel('intro',{title:q.title,tag:q.challenge?'MEISTERHERAUSFORDERUNG · '+r.name:r.name,symbol:questIcon(q),subtitle:q.npc,body:`<div class="competence-box"><span>DAS LERNST DU</span><strong>${esc(goal)}</strong><p><b>Daran erkennst du es:</b> ${esc(criterion)}</p></div><div class="intro-layout"><div class="story-medallion">${icon(questIcon(q))}</div><div class="story-text"><p>${esc(q.story)}</p><div class="reward-line">${icon(done?'check':'spark')}<span>${esc(done?'Schon geschafft: '+q.reward:q.reward)}</span></div></div></div><div class="quest-meta"><span class="mini-tag">${icon(q.advanced?'star':'flag')}${q.challenge?'Besondere Herausforderung':q.advanced?'Große Quest':'Kleine Quest'}</span><span class="mini-tag">${q.tasks.length} Aufgaben</span><span class="mini-tag">${esc(r.topic)}</span></div>${placed?'<p class="placement-note"><strong>Standortcheck bestanden.</strong> Die Grundlagen dieses Gebiets gelten für die Navigation als nachgewiesen. Diese Quest bleibt freiwillig zum Üben; sie wird nicht als erledigt markiert.</p>':''}${q.challenge?'<p class="calm-note">Freiwillige Vertiefung: Diese Probleme sind anspruchsvoller als die regulären Aufgaben. Du darfst Hilfen nutzen oder später zurückkehren. Die Herausforderung blockiert keinen weiteren Lernweg.</p>':''}${done?'<p class="replay-note">Du kannst diese Quest wiederholen. Dein Bauwerk und dein Fortschritt bleiben erhalten.</p>':progress?`<p class="replay-note">${progress.ready?q.tasks.length:progress.step} von ${q.tasks.length} Schritten geschafft. ${progress.ready?'Dein Bauwerk wartet auf deinen Abschluss.':'Du machst genau dort weiter.'}</p>`:''}`,foot:secondary('Offene Werkstatt','introWorkshop','route')+secondary('Wissen lesen','introInfo','book')+primary(done?'Noch einmal üben':placed?'Freiwillig üben':progress?.ready?'Zum Bauabschluss':progress?'Weiterlernen':'Quest beginnen','beginQuest')});
 $('introWorkshop').onclick=()=>showWorkshop(q.region,()=>showQuestIntro(q));
...
function resetAnswer(){
 ...
 session.answer=t.type==='number'?'':['multi','order','argument'].includes(t.type)?[]:['classify','match'].includes(t.type)?Array(t.items.length).fill(null):null;
...
function taskControl(t) {
 const a=session.answer,widget=widgetHTML(t,a,session.solved,session.seed);if(widget!==null)return widget;
...
function showKnowledge(id,automatic=false,contextQuest=session?.q){
 ...
 openPanel('lesson',{title:l.title,tag:automatic?'EIN GEDANKE VOR DER AUFGABE':'GENAU DAS WISSEN FÜR DIESEN SCHRITT',symbol:'book',className:'lesson-panel',body:`<div class="concept-layout"><p class="concept-text">${esc(l.text)}</p>${labHTML(l.id)}</div>`,foot:secondary('Ein Beispiel lesen','knowledgeExample','eye')+primary(automatic?'Jetzt ausprobieren':session?'Zur Aufgabe':'Zurück','knowledgeDone')});
...
function showResident(o){
 const ev=evidence(),qs=basicQuests(o.region),next=qs.find(q=>!questSatisfied(q)&&questUnlocked(q,ev))||qs.find(q=>!questSatisfied(q))||questById('master-'+o.region);
 openPanel('resident',{title:o.name+' hat einen Wunsch',tag:regionById(o.region).name,symbol:'leaf',body:`<div class="story-text"><p>${esc(next.story)}</p><p>${allBasicDone(o.region,evidence())?'Du hast allen regulären Projekten geholfen. Beim Meisterkristall wartet etwas Besonderes.':'Die hellen Zeichen zeigen dir empfohlene nächste Projekte. Blasse Zeichen darfst du ebenfalls versuchen.'}</p></div>`,foot:secondary('Zurück','residentBack')+primary('Zum Projekt gehen','residentWish')});
...
function showExamTask(){
 ...
 bindSpecialWidgets(t);$('pauseExam').onclick=closeToWorld;
...
function completeGateRound(){
 ...
 learning.emit('exam_round_finished',{correct:result.correct,total:result.total,complete:result.complete});if(result.complete)learning.complete(s.q,s);
 if(result.complete){
 ...
 openPanel('gate-result',{title:'Einige Gedanken brauchen noch Übung',tag:'PRÜFUNGSERGEBNIS',symbol:'shield',className:'exam-panel',body:`<div class="test-result-score"><strong>${result.correct} / ${result.total}</strong><span>in dieser Runde richtig</span></div><p>Die Blockade bleibt noch stehen. Richtige Nachweise sind gespeichert. In der nächsten Runde werden nur die ${result.failed.length} noch offenen Gedanken geprüft.</p><div class="exam-topics">${result.failed.map(id=>`<span>${esc(lessonById(id).title)}</span>`).join('')}</div><p class="small-note">Hier werden keine Lösungen verraten. Du kannst den Test verlassen, im Gebiet weiterlernen und anschließend einen neuen Versuch machen.</p>`,foot:secondary('Im Gebiet weiterlernen','leaveExam')+primary('Neue Prüfrunde beginnen','retryExam','shield')});
...
function placementAnswer(t){return t.type==='number'?'':null;}
function startPlacement(region,back=()=>showLearningMode(region)){
 const tasks=placementTasks(region,QUESTS);if(tasks.length<3){toast('Für diesen Bereich ist noch kein Standortcheck verfügbar.');back();return;}
 placementSession={region,tasks,index:0,answer:placementAnswer(tasks[0]),results:[],checked:false,back};showPlacementTask();
}
function placementControl(t,s){
 if(t.type==='number')return `<div class="placement-number"><p class="question">${esc(t.text)}</p>${mathVisual(t.visual,true)}<label>Deine Antwort <span class="number-entry"><input id="placementNumber" type="text" inputmode="decimal" value="${esc(s.answer)}" autocomplete="off"><span class="unit">${esc(t.unit||'')}</span></span></label></div>`;
 return `<p class="question">${esc(t.text)}</p>${mathVisual(t.visual,true)}<div class="choices">${t.options.map((option,i)=>`<button class="choice ${s.answer===i?'selected':''}" data-placement-choice="${i}" aria-pressed="${s.answer===i}"><span class="choice-symbol">${String.fromCharCode(65+i)}</span><span>${esc(option)}</span></button>`).join('')}</div>`;
}
function showPlacementTask(){
 const s=placementSession,t=s.tasks[s.index],r=regionById(s.region),ready=t.type==='number'?!!decimal(s.answer):Number.isInteger(s.answer);
 const feedback=s.checked?`<div class="feedback ${s.results.at(-1)?'correct':'wrong'}" role="status"><strong>${s.results.at(-1)?icon('check')+'Sicher gelöst.':'Hier ist noch Übung sinnvoll.'}</strong><p>Die Lösung wird im Standortcheck nicht gezeigt. So bleibt er ein ehrlicher Orientierungscheck.</p></div>`:'';
 openPanel('placement',{title:'Standortcheck · '+r.topic,tag:'5 KURZE AUFGABEN · KEINE NOTE',symbol:'shield',className:'placement-panel exam-panel',body:`<div class="competence-box"><span>FACHLICHES ZIEL</span><strong>${esc(REGION_GOALS[s.region])}</strong><p>Der Check entscheidet nur über Empfehlungen und freiwilliges Überspringen. Er ersetzt keine schulische Leistungsfeststellung.</p></div><div class="step-line"><span>Aufgabe ${s.index+1} von ${s.tasks.length}</span><span class="exam-no-help">ohne Hinweise</span></div>${placementControl(t,s)}${feedback}`,foot:secondary('Abbrechen','placementBack')+primary(s.checked?(s.index===s.tasks.length-1?'Auswerten':'Nächste Aufgabe'):'Antwort prüfen','placementNext','check',!s.checked&&!ready)});
 $('placementBack').onclick=()=>{const back=s.back;placementSession=null;back();};
 if(t.type==='number'&&!s.checked){const input=$('placementNumber');input.oninput=e=>{s.answer=e.target.value;$('placementNext').disabled=!decimal(s.answer);};setTimeout(()=>input.focus(),0);}
 panel.querySelectorAll('[data-placement-choice]').forEach(b=>b.onclick=()=>{if(s.checked)return;s.answer=+b.dataset.placementChoice;showPlacementTask();});
 $('placementNext').onclick=()=>{
  if(!s.checked){if(!(t.type==='number'?!!decimal(s.answer):Number.isInteger(s.answer)))return;s.results.push(checkAnswer(t,s.answer));s.checked=true;showPlacementTask();return;}
  if(s.index===s.tasks.length-1){finishPlacement();return;}s.index++;s.answer=placementAnswer(s.tasks[s.index]);s.checked=false;showPlacementTask();
 };
}
function finishPlacement(){
 const s=placementSession,score=s.results.filter(Boolean).length,total=s.tasks.length,threshold=Math.ceil(total*.8),passed=score>=threshold,r=regionById(s.region),back=s.back;
 state.placement??={};state.placement[s.region]={passed,score,total,at:new Date().toISOString()};placementSession=null;if(passed)chooseNext(s.region);world.sync(state);persist();updateHUD();
 const next=passed?(QUESTS.find(q=>q.region===s.region&&q.challenge&&questUnlocked(q,evidence()))||recommendedQuest()):recommendedQuest(s.region);
 openPanel('placement-result',{title:passed?'Grundlagen sicher gezeigt':'Hier lohnt sich gezieltes Weiterlernen',tag:'STANDORTCHECK · ORIENTIERUNG',symbol:passed?'check':'route',className:'placement-panel',body:`<div class="test-result-score"><strong>${score} / ${total}</strong><span>${passed?'Grundlagen in diesem kurzen Check sicher bearbeitet':'Aufgaben ohne Hilfe richtig'}</span></div><p>${passed?'Du kannst die regulären Quests dieses Gebiets freiwillig überspringen und direkt eine Vertiefung oder den nächsten fachlichen Schritt wählen. Die übersprungenen Quests bleiben sichtbar und werden nicht als erledigt gezählt.':'Das ist keine Note und keine Diagnose. Nutze die offenen Aufgaben als Hinweise darauf, welche Grundlagen du noch bearbeiten möchtest.'}</p><p class="calm-note"><strong>${esc(r.topic)}</strong><br>${esc(REGION_GOALS[s.region])}</p>`,foot:secondary('Zur Lernübersicht','placementOverview','book')+(next?primary(passed&&next.challenge?'Zur Vertiefung':'Passende Quest öffnen','placementNextStep','arrow'):'')});
 $('placementOverview').onclick=()=>showLearningMode(s.region);if($('placementNextStep'))$('placementNextStep').onclick=()=>showQuestIntro(next);
}
function showWorkshop(region,back=()=>showLearningMode(region)){
 const w=OPEN_TASKS[region],r=regionById(region),done=!!state.workshops?.[region]?.done;if(!w){back();return;}
 openPanel('workshop',{title:w.title,tag:'OFFENE MATHEMATIKWERKSTATT · '+r.name,symbol:'route',className:'workshop-panel',body:`<div class="competence-box"><span>ZIEL DES GEBIETS</span><strong>${esc(REGION_GOALS[region])}</strong></div><div class="workshop-card"><span class="mini-tag">${esc(w.mode)}</span><h3>Auftrag</h3><p>${esc(w.prompt)}</p><h3>Euer Ergebnis</h3><p>${esc(w.product)}</p></div><p class="calm-note"><strong>Nicht automatisch bewertet.</strong> Nutze Papier, Heft oder ein Gespräch. Die App speichert keine freie Antwort. ${done?'Du hast diese Werkstatt bereits als bearbeitet markiert.':''}</p>`,foot:secondary('Zurück','workshopBack')+primary(done?'Noch einmal bearbeiten':'Als bearbeitet markieren','workshopDone','check')});
 $('workshopBack').onclick=back;$('workshopDone').onclick=()=>{state.workshops??={};state.workshops[region]={done:true,at:new Date().toISOString()};persist();toast('Werkstatt als bearbeitet markiert. Die Begründung selbst wird nicht gespeichert.');back();};
}
function entryRouteFor(region){
 if(state.visited.includes(region)||region==='village')return null;
 for(const [a,b] of PATHS){if(b===region&&state.visited.includes(a))return {from:a,to:b,kind:'gate'};if(a===region&&state.visited.includes(b))return {from:b,to:a,kind:'gate'};}
 return null;
}
function showLearningMode(selected=world.region().id){
 const ev=evidence(),r=regionById(selected)||REGIONS[0],placement=state.placement?.[r.id],workshop=state.workshops?.[r.id],route=entryRouteFor(r.id),routeOpen=route?gateStatus(state,route.from,route.to).open:true;
 if(route&&routeOpen&&!state.visited.includes(r.id)){state.visited.push(r.id);persist();learning.enterTopic(r.id);}
 const entered=state.visited.includes(r.id)||r.id==='village',available=regionUnlocked(r.id,ev),canWork=entered&&available;
 const regular=basicQuests(r.id),done=regular.filter(q=>state.completed[q.id]).length,satisfied=regular.filter(q=>questSatisfied(q)).length;
 const qButtons=questsIn(r.id).filter(q=>!q.challenge||allBasicDone(r.id,ev)).map(q=>{const done=!!state.completed[q.id],placed=!q.challenge&&!done&&placementPassed(r.id),unlocked=questUnlocked(q,ev)&&canWork;return `<button data-learning-quest="${q.id}" class="learning-quest ${done?'done':''} ${placed?'placed':''} ${!unlocked?'locked':''}">${icon(done?'check':placed?'shield':unlocked?questIcon(q):'lock')}<span><strong>${esc(q.title)}</strong><small>${q.challenge?'Meisterherausforderung':placed?'Vorwissen nachgewiesen · freiwillige Übung':q.extension?'Zusatzweg':esc(q.goal)}</small></span><em>${done?'fertig':state.progress[q.id]?.ready?'bereit':state.progress[q.id]?`${state.progress[q.id].step}/${q.tasks.length}`:`${q.tasks.length} Aufgaben`}</em>${icon('chevron')}</button>`;}).join('');
 const gateNote=route&&!routeOpen?`<div class="learning-gate"><strong>${icon('shield')} Wegprüfung vor ${esc(r.name)}</strong><p>Du kannst den Bereich in der 2D-Ansicht erreichen, sobald du denselben fachlichen Nachweis wie am Wegtor bestehst.</p><button id="learningGate" class="secondary-button">Wegprüfung öffnen</button></div>`:!entered?`<div class="learning-gate"><strong>${icon('map')} Noch nicht erreicht</strong><p>Erreiche zuerst einen angrenzenden Lernort. Die 2D-Ansicht ersetzt keine fachlichen Wegprüfungen oder Wege zwischen den Gebieten.</p></div>`:!available?`<div class="learning-gate"><strong>${icon('lock')} Noch nicht fachlich freigeschaltet</strong><p>${esc(requirementText(r.id,ev))}</p></div>`:'';
 openPanel('learning-mode',{title:'Meine Lernübersicht',tag:'2D-LERNMODUS · OHNE HERUMLAUFEN',symbol:'book',className:'learning-mode-panel',body:`<div class="learning-region-tabs">${REGIONS.map(x=>`<button data-learning-region="${x.id}" class="${x.id===r.id?'active':''}">${icon(x.icon)}<span>${esc(x.name)}</span></button>`).join('')}</div><div class="competence-box"><span>MEIN FACHLICHES ZIEL</span><strong>${esc(REGION_GOALS[r.id])}</strong><p>${done} reguläre Quests abgeschlossen · ${satisfied} fachlich bearbeitet oder im Standortcheck nachgewiesen.</p></div>${gateNote}<div class="learning-tools"><button id="placementStart" ${!canWork?'disabled':''}><span>${icon('shield')}</span><strong>${placement?'Standortcheck wiederholen':'Standortcheck'}</strong><small>${placement?`${placement.score}/${placement.total} · ${placement.passed?'Grundlagen nachgewiesen':'Weiterlernen empfohlen'}`:'5 kurze Aufgaben · ohne Hilfen · keine Note'}</small></button><button id="workshopStart" ${!canWork?'disabled':''}><span>${icon('route')}</span><strong>Offene Werkstatt</strong><small>${workshop?'bearbeitet · jederzeit erneut möglich':'begründen · darstellen · austauschen'}</small></button></div><div class="learning-list"><h3>Quests in ${esc(r.name)}</h3>${qButtons||'<p class="calm-note">Hier sind noch keine Quests zugänglich.</p>'}</div>`,foot:secondary('3D-Welt anzeigen','learningWorld','map')+secondary('Lernplan','learningPlan2d','route')});
 panel.querySelectorAll('[data-learning-region]').forEach(b=>b.onclick=()=>showLearningMode(b.dataset.learningRegion));
 panel.querySelectorAll('[data-learning-quest]').forEach(b=>b.onclick=()=>{const q=questById(b.dataset.learningQuest);if(!canWork||!questUnlocked(q,evidence())){if(route&&!routeOpen)showGateIntro(route);else showLocked(q);return;}showQuestIntro(q);});
 $('placementStart').onclick=()=>startPlacement(r.id,()=>showLearningMode(r.id));$('workshopStart').onclick=()=>showWorkshop(r.id,()=>showLearningMode(r.id));$('learningWorld').onclick=closeToWorld;$('learningPlan2d').onclick=()=>srlUI.planning({amend:true});if($('learningGate'))$('learningGate').onclick=()=>showGateIntro(route);
}
...
function showMap(selected=world.region().id) {
 returnFromInfo=null;
 const ev=evidence(),r=regionById(selected),p=mapPoint(world.player),available=regionUnlocked(selected,ev),placement=state.placement?.[selected],canWorkHere=available&&(state.visited.includes(selected)||selected===world.region().id);
 ...
 const nodes=REGIONS.map(n=>{
  const xy=mapPoint(n),unlocked=regionUnlocked(n.id,ev),done=questsIn(n.id).filter(q=>state.completed[q.id]).length,placed=placementPassed(n.id);
  return `<g class="map-node ${n.id===selected?'active':''} ${unlocked?'':'locked'} ${placed?'placed':''}" transform="translate(${xy.x} ${xy.y})" data-region="${n.id}" tabindex="0" role="button" aria-label="${esc(n.name)}, ${esc(n.topic)}, ${done} Quests geschafft${placed?', Standortcheck bestanden':''}"><rect class="node-disc" x="-19" y="-23" width="38" height="38" rx="9" fill="${n.color}"/><path d="M-5 15 0 22 5 15" fill="${n.color}"/><svg x="-11" y="-16" width="22" height="22" viewBox="0 0 24 24" class="node-icon">${icon(n.icon)}</svg><text class="node-title" text-anchor="middle" y="35">${esc(n.name)}</text><text class="node-sub" text-anchor="middle" y="47">${placed?'Vorwissen ✓':done+'/'+questsIn(n.id).length+' '+(unlocked?'✦':'· später')}</text></g>`;
 }).join('');
 const quests=questsIn(selected).filter(q=>!q.challenge||allBasicDone(selected,ev)).map(q=>{const done=!!state.completed[q.id],placed=!q.challenge&&!done&&placementPassed(selected),unlocked=questUnlocked(q,ev);return `<button data-map-quest="${q.id}" class="${done?'done':!unlocked?'recommended-later':''} ${placed?'placed':''} ${q.challenge?'master-row':''}">${icon(done?'check':placed?'shield':unlocked?questIcon(q):'lock')}<span>${esc(q.title)}${q.challenge?'<em>MEISTERHERAUSFORDERUNG</em>':placed?'<em>VORWISSEN NACHGEWIESEN · FREIWILLIG</em>':q.extension?'<em>ZUSATZWEG</em>':''}</span><small>${done||state.progress[q.id]?.ready?q.tasks.length:(state.progress[q.id]?.step||0)}/${q.tasks.length}</small>${icon('chevron')}</button>`;}).join('');
 const body=`<div class="map-body"><svg class="map-art landscape-map" viewBox="0 0 705 430" role="group" aria-label="Zusammenhängende Landschaft mit Küste, Wäldern, Bergzügen und acht Lernorten"><rect width="705" height="430" fill="#b6d4d0" rx="14"/><polygon points="${mapPolygon(OUTLINE)}" fill="#a4bd97" stroke="#e3dcb5" stroke-width="8"/>${clearings}${scenery}${lines}${nodes}<circle class="player-dot" cx="${p.x}" cy="${p.y}" r="5"/><text x="20" y="28" fill="#5f827c" font-size="10" letter-spacing="2">N ↑</text><text x="38" y="419" fill="#5f827c" font-size="10" letter-spacing="2">DIE SÜDKÜSTE</text></svg><div class="map-selection"><div><span class="section-tag">${icon(r.icon)}${esc(r.tag)}</span><h3>${esc(r.name)}</h3><p>${esc(r.topic)}</p><div class="map-goal"><strong>Fachliches Ziel</strong><span>${esc(REGION_GOALS[selected])}</span></div><small>${esc(available?r.hint:requirementText(selected,ev))}</small>${placement?`<small class="placement-summary">Standortcheck: ${placement.score}/${placement.total} · ${placement.passed?'Grundlagen nachgewiesen':'Weiterlernen empfohlen'}</small>`:''}<div class="map-didactic-actions"><button id="mapPlacement" ${!canWorkHere?'disabled':''}>${icon('shield')} Standortcheck</button><button id="mapWorkshop" ${!canWorkHere?'disabled':''}>${icon('route')} Offene Werkstatt</button></div></div><div class="map-quests">${quests}</div></div></div>`;
 openPanel('map',{title:'Deine Lernlandkarte',tag:`${seals()} VON 8 GEBIETSSIEGELN`,symbol:'map',subtitle:'Freie Lernwege, transparente Lernziele, Standortchecks und offene Werkstätten.',className:'map-panel',body,foot:secondary('2D-Lernübersicht','mapLearningMode','book')+primary('Zum Infoschild','walkInfo','route')});
 ...
 $('mapPlacement').onclick=()=>{if(canWorkHere)startPlacement(selected,()=>showMap(selected));};$('mapWorkshop').onclick=()=>{if(canWorkHere)showWorkshop(selected,()=>showMap(selected));};$('mapLearningMode').onclick=()=>showLearningMode(selected);$('walkInfo').onclick=()=>walkToObject(world.objects.find(o=>o.id==='info-'+selected));
}
...
function showJournal(){
 openPanel('journal',{title:'Dein Reisebuch',tag:'DEIN LERNWEG',symbol:'book',body:`<div class="journal-grid">${REGIONS.map(r=>{const qs=questsIn(r.id),done=qs.filter(q=>state.completed[q.id]).length;return `<button class="journal-card" data-journal="${r.id}">${icon(r.icon)}<h3>${esc(r.name)}</h3><strong>${done} von ${qs.length} Quests</strong><small>${esc(r.topic)}</small><span>${state.completed['master-'+r.id]?'Meisterherausforderung geschafft · der Ort blüht auf':state.mastery[r.id]?'Gebietstest bestanden · Siegel erhalten':'Gebietstest noch offen'}</span></button>`;}).join('')}</div><p class="small-note">Jeder gelöste Schritt zählt. Fehler gehören zum Lernen und nehmen dir nichts weg. Deine Infoschilder sind jederzeit offen.</p>`,foot:secondary('Zum Rucksack','journalBack','menu')+primary('Weiterreisen','journalDone')});
...
function showWelcome(){
 ...
 $('welcomeStart').onclick=()=>{state.tutorial=true;persist();enterLearning();toast('Das Infoschild neben dir hilft dir beim Start. Beginne danach bei der kleinen Weglaterne.');};
}
...
$('learnButton').onclick=()=>showLearningMode();$('mapButton').onclick=()=>showMap();$('menuButton').onclick=showMenu;
...
$('brandIcon').innerHTML=icon('gem');$('learnButton').innerHTML=icon('book');$('mapButton').innerHTML=icon('map');$('menuButton').innerHTML=icon('menu');
