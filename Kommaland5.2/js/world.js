...
import {learningEvidence} from './didactics.js';
const TAU=Math.PI*2;
...
export class World {
 ...
 buildResidents(){
  this.bees=[];const hive=QUESTS.find(q=>q.id==='m3'),mr=regionById('mill');for(let i=0;i<5;i++)this.bees.push({mesh:this.renderer.mesh(beeGeometry(),{visible:false}),x:mr.x+hive.x,z:mr.z+hive.z,phase:i*1.25});
 ...
 sync(state){
  this.state=state;const evidence=learningEvidence(state,QUESTS);let navigationChanged=false;
  for(const o of this.objects)if(o.type==='quest'){
   const done=!!state.completed[o.id],hidden=o.q.challenge&&!challengeVisible(o.region,evidence);
   if(o.hidden!==hidden){navigationChanged=true;o.hidden=hidden;o.obstacle.active=!hidden;}
   o.done=done;o.unlocked=questUnlocked(o.q,evidence);
  ...
  for(const a of this.adornments){const done=!!state.completed['master-'+a.region];a.mesh.visible=done;a.mesh.reveal=1000;a.mesh.opacity=1;a.rainbow.visible=done;a.rainbow.opacity=.62;}
 ...
 update(dt,time,paused=false){
  ...
  for(const b of this.bees){b.mesh.visible=!!this.state?.completed.m3;const t=this.state?.settings.reducedMotion?b.phase:time*1.2+b.phase;b.mesh.x=b.x+Math.cos(t)*2;b.mesh.z=b.z+Math.sin(t)*1.5;b.mesh.y=terrainHeight(b.mesh.x,b.mesh.z)+1.8+Math.sin(t*2)*.3;}
