import {SRL_RULES,SUPPORT_DEFAULTS,supportLevels,latestPlan,promptBudget,repeatedErrors,multipleSkips,rapidExits,fadeDecision,sessionReview,previousIntention} from '../teacher-dashboard/shared/regulation.js';
...
export class Regulation {
 ...
 candidate(reason){if(!this.sessionId||this.log.current?.finishedAt||!this.budget().allowed)return null;const es=this.log.events,sid=this.sessionId,ctx=this.context();
  if(reason==='errors'&&this.levels.strategy>0&&repeatedErrors(es,ctx.taskRunId,ctx.kind)&&!es.some(e=>e.sessionId===sid&&e.type==='srl_prompt_shown'&&e.taskRunId===ctx.taskRunId&&e.data.reason==='errors'))return {reason,context:ctx};
