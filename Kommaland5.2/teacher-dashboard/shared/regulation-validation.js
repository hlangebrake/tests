import {GOALS,NEXT_GOALS,STRATEGIES,SUPPORT_DEFAULTS,SRL_EVENTS,SKIP_REASONS} from './regulation.js';
...
export function validateRegulationEvent(e,assert){if(!Object.hasOwn(SRL_EVENTS,e.type))return;const d=e.data,one=(x,v)=>v.includes(x),confidence=x=>x===null||Number.isInteger(x)&&x>=1&&x<=5;
 ...
 if(e.type==='srl_prompt_shown')assert(typeof d.promptId==='string'&&one(d.reason,['errors','skips','orientation','new_topic','area_exit','requested'])&&typeof d.unsolicited==='boolean','Ungültiger Regulationsimpuls.');
