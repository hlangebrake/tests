...
export function validateCatalog(c){
 ...
 out.quests=c.quests.map(x=>{assert(plain(x)&&validId(x.questId)&&!quests.has(x.questId)&&topics.has(x.topicId)&&['regular','challenge'].includes(x.kind),'Ungültige Katalogquest.');quests.set(x.questId,x.topicId);return {questId:x.questId,topicId:x.topicId,title:label(x.title),kind:x.kind};});
 out.tasks=c.tasks.map(x=>{assert(plain(x)&&validId(x.taskId)&&!tasks.has(x.taskId)&&quests.get(x.questId)===x.topicId&&validId(x.skillId)&&validId(x.type),'Ungültige Katalogaufgabe.');tasks.add(x.taskId);return {taskId:x.taskId,questId:x.questId,topicId:x.topicId,skillId:x.skillId,skillTitle:label(x.skillTitle),type:x.type,title:label(x.title)};});
 return out;
}
export function validateSave(raw){
 ...
 assert([1,SCHEMA_VERSION].includes(raw.schemaVersion),`Nicht unterstützte Lernstand-Version: ${String(raw.schemaVersion)}.`);
 ...
 const catalog=validateCatalog(raw.catalog),knownQuests=new Set(catalog.quests.map(q=>q.questId)),knownTopics=new Set(catalog.topics.map(t=>t.topicId));
 ...
 if(raw.currentState.placement!==undefined){
  assert(plain(raw.currentState.placement)&&Object.keys(raw.currentState.placement).length<=100,'Ungültige Standortcheckdaten.');
  for(const [id,v] of Object.entries(raw.currentState.placement))assert(knownTopics.has(id)&&plain(v)&&typeof v.passed==='boolean'&&Number.isInteger(v.score)&&Number.isInteger(v.total)&&v.total>=1&&v.total<=20&&v.score>=0&&v.score<=v.total&&validDate(v.at),'Ungültiger Standortcheck im Spielstand.');
 }
 if(raw.currentState.workshops!==undefined){
  assert(plain(raw.currentState.workshops)&&Object.keys(raw.currentState.workshops).length<=100,'Ungültige Werkstattdaten.');
  for(const [id,v] of Object.entries(raw.currentState.workshops))assert(knownTopics.has(id)&&plain(v)&&v.done===true&&validDate(v.at),'Ungültige Werkstatt im Spielstand.');
 }
