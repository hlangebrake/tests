...
export function makeDemoSave(index=0){
 ...
 const state={app:'kommaland',schemaVersion:3,contentVersion:6,createdAt:'2026-09-01T07:00:00.000Z',updatedAt:'2026-09-10T10:00:00.000Z',player:{x:0,z:24},completed:{},progress:{},mastery:{},competencies:{},exams:{},visited:['village'],read:[],activeQuest:'c6-v1',tutorial:true,settings:{sound:false,quality:'normal',largeText:false,reducedMotion:false},stats:{attempts:0,correct:0,hints:0},encounters:{count:0,distance:0},learner:{studentId,studentName}};
 ...
 const count=[9,7,12,5,16,14,11,3,10,8,15,6][index%12],regular=CATALOG.quests.filter(q=>q.active!==false&&q.kind==='regular');
 ...
 for(const q of CATALOG.quests.filter(q=>q.active!==false&&q.kind==='challenge'&&regular.filter(x=>x.topicId===q.topicId).every(x=>state.completed[x.questId]))){ctx={questId:q.questId,topicId:q.topicId,kind:'challenge',runId:uid()};emit('challenge_unlocked');if(index!==4)continue;emit('challenge_started');for(const t of CATALOG.tasks.filter(t=>t.questId===q.questId)){ctx={...ctx,taskId:t.taskId,taskRunId:uid(),skillId:t.skillId};emit('task_started',{taskType:t.type,taskTitle:t.title});slice(22000);emit('task_attempt',{attemptNumber:1,correct:true});emit('task_completed');}ctx={questId:q.questId,topicId:q.topicId,kind:'challenge',runId:ctx.runId};const e=emit('challenge_completed');state.completed[q.questId]={at:e.timestamp,mistakes:0,hints:0};}
