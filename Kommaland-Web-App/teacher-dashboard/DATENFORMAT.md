# Gemeinsames Lernstandsformat · Version 3

## Verbindliche Quelle

`shared/save-schema.js` wird von Schüler-App und Lehrkräfte-App importiert.
`SCHEMA_VERSION = 3`, `APP_VERSION = "6.0.0"`,
...
## Inhaltsschema 6: historische IDs bleiben historisch

Die Austauschversion ist 3, `currentState.version` bleibt 3. `currentState.contentVersion`
ist 6. Die fachliche Migration steht in der Schüler-App in `js/content-migration.js`.
`contentArchive` erhält alte Abschluss-/Teilstände, Gebietssiegel und pausierte Tests.

Aktive Quests haben `c6-…`-IDs, aktive Aufgaben `c6:…`-Schlüssel. Der Katalog umfasst
24 aktive und 45 archivierte Quests sowie 101 aktive und 393 archivierte Aufgaben.
`active: false` markiert Archivmetadaten, nicht die Löschung alter Ereignisse. Quoten
berücksichtigen die am Stichtag aktive Inhaltsversion. Historische Kennzahlen nutzen
historische Kataloge, nicht rückwirkend den neuen Nenner.

`curriculum_changed` hält den fachlichen Versionswechsel und angerechnete Abschlüsse
fest. Alte richtig beantwortete Aufgaben werden niemals auf neue `taskId`-Werte
umgeschrieben. Anrechnung erzeugt keine `task_attempt`-Ereignisse. Vollständige historische
Gruppen gelten als Abschluss; offene Gruppen beginnen in der neuen Folge neu.

`srl_strategy_review` hält ausschließlich gewählte Rückschau-Kategorien fest.
`srl_prompt_answered` und weitere Strategieereignisse können auf den tatsächlich
betroffenen Grundlagen-Gedanken verweisen. Rohdaten bleiben die Grundlage der Kennzahlen.

Parser nehmen weiterhin Austauschversionen 1 und 2 an. Ältere Dashboards verstehen das
neue Format nicht vollständig und müssen mit der Schüler-App aktualisiert werden.
Bestehende Datenbank-Stores und stabile Personen-/Ereignis-IDs bleiben unverändert.
