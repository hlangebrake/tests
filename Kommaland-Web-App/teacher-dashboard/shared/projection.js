...
export function project({studentId,catalog,events,snapshots=[],asOf='9999-12-31T23:59:59.999Z'}){
 ...
 for(const e of es){
  ...
  if(e.type==='task_started'&&t){
   ...
   if(!runs.has(e.taskRunId)){const r={taskRunId:e.taskRunId,taskId:e.taskId,questId:e.questId,topicId:e.topicId,skillId:e.skillId,kind:kindOf(e),sessionId:e.sessionId,runId:e.runId,startedAt:e.timestamp,finishedAt:null,duration:0,attempts:[],helps:[],correct:null,status:'started'};runs.set(e.taskRunId,r);t.runs.push(r);}
  }
 ...
 if(snap?.currentState){
  ...
  for(const q of qs.values()){
   ...
   if(state.completed[q.questId]){q.status='completed';q.completedAt=state.completed[q.questId].at||q.completedAt;}
   else if(state.progress[q.questId])q.status=q.status==='skipped'?'skipped':'started';
   ...
   if(q.kind==='challenge'){q.challengeCompleted=!!state.completed[q.questId];q.challengeStarted=q.challengeStarted||!!state.progress[q.questId];q.challengeUnlocked=q.challengeUnlocked||catalog.quests.filter(x=>x.kind==='regular'&&x.topicId===q.topicId).every(x=>!!state.completed[x.questId]);}
  }
 }
