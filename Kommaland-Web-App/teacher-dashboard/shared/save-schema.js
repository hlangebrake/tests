import {DIFFICULTY_EVENTS,validateDifficultyEvent,validateDifficultyHistory,cleanQuestLevels,cleanTaskVariants} from './difficulty.js';
import {V7_EVENTS,validateV7Event,validateV7History} from './learning7.js';
 ...
 * Automatic-task answers, names in events, device IDs and IP addresses are not recorded.
 * Optional open-work notes (max. 1200 characters) are explicitly submitted, never graded.
 */
...
for(const type of V7_EVENTS)EVENT_TYPES.add(type);
for(const type of DIFFICULTY_EVENTS)EVENT_TYPES.add(type);
...
export function validateCatalog(c){
 ...
 out.tasks=c.tasks.map(x=>{assert(plain(x)&&validId(x.taskId)&&!tasks.has(x.taskId)&&quests.get(x.questId)===x.topicId&&validId(x.skillId)&&validId(x.type),'Ungültige Katalogaufgabe.');tasks.add(x.taskId);return {taskId:x.taskId,questId:x.questId,topicId:x.topicId,skillId:x.skillId,skillTitle:label(x.skillTitle),type:x.type,title:label(x.title),...(x.level&&['I','II','III'].includes(x.level)?{level:x.level}:{}),...(Array.isArray(x.processes)&&x.processes.length<=10&&x.processes.every(validId)?{processes:x.processes.slice()}:{}),...(x.active!==undefined?{active:x.active===true,contentVersion:x.contentVersion===6?6:5}:{})};});
 return out;
}
export function validateSave(raw){
 ...
 assert([1,2,3,4,SCHEMA_VERSION].includes(raw.schemaVersion),`Nicht unterstützte Lernstand-Version: ${String(raw.schemaVersion)}.`);
 ...
 const events=raw.events.map(e=>{
  ...
  if(e.kind!=null){assert(['regular','challenge','exam','encounter','practice'].includes(e.kind),'Ungültige Aufgabenart.');result.kind=e.kind;}
  ...
  validateV7Event(result,assert);
  validateDifficultyEvent(result,assert);
  return result;
 });
 validateV7History(events,assert);
 validateDifficultyHistory(events,assert);
 cleanQuestLevels(raw.currentState.questLevels);
 for(const [q,p] of Object.entries(raw.currentState.progress||{}))cleanTaskVariants(p.taskVariants,q);
