...
**Version 3.0.0 · Interaktive Schaubilder**

8 Lernorte, 37 Quests, 122 feste Teilaufgaben, 83 kurze Wissensschritte mit interaktiven Schaubildern und 83 zugeordnete Beispiele mit jeweils drei aufdeckbaren Denkschritten. Das Paket ist statisch hostbar: HTML, CSS und JavaScript, prozedurale WebGL-Grafik, keine Laufzeit-Bibliotheken, kein CDN, keine Anmeldung und kein Backend.

## Neu in Version 3

Jede der 83 Wissenskarten enthält jetzt ein eigenes parametriertes, interaktives Schaubild. Die 38 Modellfamilien passen zu den Lernideen: gleich große Teile färben, Ziffern an Stellen verändern, Zahlengeraden ziehen, Rundungsgrenzen untersuchen, Mengen umbenennen, schrittweise bündeln/entbündeln, Rechtecke aufziehen und gleiche Portionen bilden. Die Erkundungsfrage steht direkt am Modell; Zahl und Erklärung aktualisieren sich sofort.

„Zurücksetzen“ setzt nur das Schaubild zurück. Regler und große ±-Tasten ergänzen kleine Zieh- oder Malflächen. Die Modelle funktionieren lokal ohne zusätzliche Bibliotheken oder Netzabrufe. Die mathematischen Beziehungen und Einheiten wurden redaktionell überprüft; konkrete Korrekturen, Grenzen und Tests stehen in `MATHEMATIK-PRUEFUNG.md`. Der vollständige Plan aller 83 Karten steht in `SCHAUBILDER.md`.

**Quests, Aufgaben-IDs und Spielstand-Schema sind gegenüber Version 2 unverändert.** Version-2-Spielstände werden ohne Verlust bereits geprüfter Teilschritte übernommen. Landschaft, Aufbauanimationen, Beispielrollen und Export/Import bleiben bestehen.

## Bereits seit Version 2
...
js/visuals.js      js/visual-models.js
```

**Insbesondere die neuen Dateien `js/visuals.js` und `js/visual-models.js` mit hochladen; auch `styles.css`, `lessons.js`, `app.js`, `ui.js` und `sw.js` ersetzen.** Ein Unterverzeichnis ist möglich. Keine Installation von npm-Paketen und kein Build-Schritt sind zum Betrieb nötig. Der Host muss JavaScript mit einem passenden MIME-Typ ausliefern.
...
**Zuerst auf jedem benutzten Gerät den Spielstand im Rucksack als JSON exportieren.** Danach alle Laufzeitdateien gemeinsam ersetzen, nicht nur die HTML-Datei. Die neue Cache-Version endet auf `v3.0.0` (mit installationsbezogenem Präfix). Bereits geöffnete Seiten können noch alten JavaScript-Code ausführen: nach dem Update alle noch geöffneten Kommaland-Tabs und Web-App-Fenster schließen und die neue Version öffnen. Alte und neue Version nicht gleichzeitig im selben Installationsordner benutzen.

**Beim Wechsel von Version 2 auf Version 3 bleibt der vollständige Spielstand in Schema 2 erhalten, einschließlich offener Teilschritte und unbestätigter Bauabschlüsse.** Im Rucksack ist die neue Oberfläche an „Kommaland 3.0“ zu erkennen. Verwende dieselbe HTTPS-Adresse und dasselbe Unterverzeichnis wie bisher; eine neue Adresse hat einen anderen lokalen Speicherbereich.

Die lokale Speicherkennung bleibt absichtlich auch mit Version 1 kompatibel. Version-1-Dateien werden beim Lesen in Schema 2 umgewandelt. Abgeschlossene Quests und deren Bauwerke bleiben erhalten; die neu eingefügten Quests bleiben offen. **Angefangene, aber nicht abgeschlossene Aufgaben aus Version 1 starten neu**, weil Reihenfolge und Inhalte verändert wurden. Die App erklärt das bei automatischer Migration beziehungsweise vor dem Import. Der Einstieg wird auf die neue Weglaterne gesetzt. Alte Positionen werden auf begehbares Gelände versetzt, falls nötig.
...
Die vorübergehenden Einstellungen eines Schaubilds bleiben in der laufenden Sitzung beim Öffnen eines Beispiels erhalten. Sie sind kein gespeicherter Lernfortschritt und werden nicht exportiert. Ein Seitenneustart, ein neues Abenteuer oder das Laden eines anderen Spielstands setzt sie zurück.
...
Inhalte ändern: `js/content.js` enthält Aufgaben, Antwortschlüssel und konkrete Voraufgaben; `js/lessons.js` die Wissens-/Beispielkarten. `js/visual-models.js` ordnet jeder Wissens-ID ein Modell und einen Zahlenraum zu; `js/visuals.js` rendert Modelle und verarbeitet Pointer-/Tastatureingaben. `js/terrain.js` bestimmt Landfläche, Lichtungen und Wege gemeinsam für Welt, Kollision und Karte. `js/world.js` enthält Modelle, Navigation und Aufbauanimation; `styles.css` gestaltet die Oberfläche. Nach Inhaltsänderungen mit anderer Aufgabenreihenfolge die Spielstand-Migration prüfen; nach Veröffentlichung den Cache-Namen in `sw.js` ändern.
...
xvfb-run -a python tests/visual_browser.py
...
`SCHAUBILDER.md` – jede Wissenskarte mit Interaktion und didaktischer Absicherung.  
`MATHEMATIK-PRUEFUNG.md` – Korrekturen, Rechenmodelle und Grenzen der Prüfung.  
