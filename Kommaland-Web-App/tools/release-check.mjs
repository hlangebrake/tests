...
assert.ok(sw.includes("CACHE_PREFIX+'v4.1.0'"));
...
assert.equal(fs.readdirSync(path.join(root,'js')).filter(n=>n.endsWith('.js')).length,19);
...
for(const report of ['browser-regression-results.json','browser-presentation-results.json','visual-browser-results.json','model-gallery-results.json']){
 assert.deepEqual(JSON.parse(fs.readFileSync(path.join(root,'tests',report),'utf8')).errors,[],report);
}
assert.ok(fs.readFileSync(path.join(root,'tests/node-results.txt'),'utf8').includes('# pass 87'));
const result={version:pkg.version,syntaxCheckedFiles:files.length,relativeModuleImportsChecked:imports,javascriptRuntimeModules:19,serviceWorkerFileEntries:cacheFiles.length,quests:QUESTS.length,tasks:QUESTS.reduce((n,q)=>n+q.tasks.length,0),lessons:Object.keys(LESSONS).length,modelTypes:new Set(QUESTS.map(q=>q.kind)).size,nodeTests:87,browserReportsWithoutPageErrors:4,contentLocks:'pass',status:'pass',limits:'Static checks and documented local browser harness only; no physical iPad/Safari or real offline installation test.'};
