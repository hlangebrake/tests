...
test('24 eindeutige Quests, 101 vollständige Aufgaben und 24 Kernkarten',()=>{
 assert.equal(QUESTS.length,24);assert.equal(new Set(QUESTS.map(q=>q.id)).size,24);
 assert.equal(QUESTS.reduce((n,q)=>n+q.tasks.length,0),101);
 assert.equal(REGIONS.reduce((n,r)=>n+INFO[r.id].length,0),24);
 const types=new Set();
 for(const q of QUESTS){assert.ok(REGIONS.some(r=>r.id===q.region));assert.ok(q.story&&q.reward&&q.npc);assert.ok(q.tasks.length>=3&&q.tasks.length<=15);
  for(const t of q.tasks){types.add(t.type);assert.ok(t.text&&t.hint&&t.why);assert.ok(checkAnswer(t,t.type==='mark'?Array.from({length:t.answer},(_,i)=>i):t.answer),q.id+': '+t.text);
...
test('Der Lernweg ist ohne Zirkelschluss bis zum Finale lösbar',()=>{
 ...
 assert.equal(questUnlocked(QUESTS.find(q=>q.id==='c6-f1'),done),false);assert.equal(regionUnlocked('village',done),true);
...
test('Waldbegegnungen beziehen sich nur auf tatsächlich abgeschlossene Aufgaben',()=>{
 for(let i=0;i<30;i++){const q=makeEncounter({'c6-v1':true,'c6-a1':true},i);assert.ok(['village','market'].includes(q.region));assert.ok(checkAnswer(q.tasks[0],q.tasks[0].answer));}
});
