...
test('33 eindeutige Quests, 99 vollständige Aufgaben und 24 Wissensseiten',()=>{
 assert.equal(QUESTS.length,33);assert.equal(new Set(QUESTS.map(q=>q.id)).size,33);
 assert.equal(QUESTS.reduce((n,q)=>n+q.tasks.length,0),99);
 assert.equal(REGIONS.reduce((n,r)=>n+INFO[r.id].length,0),24);
 const types=new Set();
 for(const q of QUESTS){assert.ok(REGIONS.some(r=>r.id===q.region));assert.ok(q.story&&q.reward&&q.npc);assert.equal(q.tasks.length,3);
...
test('Waldbegegnungen beziehen sich nur auf bereits besuchte Lerninhalte',()=>{
