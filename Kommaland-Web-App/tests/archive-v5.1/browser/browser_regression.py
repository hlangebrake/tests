...
Run: xvfb-run -a python3 tests/browser_regression.py
"""
...
async def main():
 ...
 async with async_playwright() as p:
  ...
  await page.locator('#srlPlanSkip').click()
  ...
  print('Starting 393-task regression',flush=True)
  result=await page.evaluate('''async()=>{
   ...
   for(const q of d.content.QUESTS){
    ...
    for(let n=0;n<q.tasks.length;n++){
     ...
     count++;types[t.type]=(types[t.type]||0)+1;await d.learning.flush();
    ...
    document.getElementById('backWorld').click();await d.learning.flush();console.log('Completed '+q.id+'; '+count+' tasks; '+d.learning.events.length+' events');
   }
  ...
  report['learningExport']=await page.evaluate('async()=>{const d=await __kommaland.learning.exportSave(true);return {studentId:!!d.studentId,events:d.events.length,sessions:d.sessions.length,attempts:d.events.filter(e=>e.type==="task_attempt").length,taskProgress:d.taskProgress.length,valid:true};}')
  assert not report['errors'],report['errors']
  await browser.close()
 (ROOT/'tests/browser-regression-results.json').write_text(json.dumps(report,ensure_ascii=False,indent=2))
