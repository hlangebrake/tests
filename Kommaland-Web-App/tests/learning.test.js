...
test('Der Dorfeinstieg baut ganze Zahlen, Hälfte, Zehntel, Hundertstel und Brüche getrennt auf',()=>{
 const ids=QUESTS.filter(q=>q.region==='village'&&!q.challenge).map(q=>q.id);assert.deepEqual(ids,['v0a','v0b','v1','v0c','v2','v0d','v3','v4']);
...
test('Voraussetzungen sind konkrete Aufgaben; es gibt keine unbekannten oder zyklischen Kanten',()=>{
 ...
 QUESTS.forEach(visit);assert.equal(visited.size,45);
});
test('Kernroute erreicht das Finale ohne die beiden gekennzeichneten Zusatzwege',()=>{
 ...
 const done={};let changed=true;while(changed){changed=false;for(const q of QUESTS)if(!q.extension&&!q.challenge&&!done[q.id]&&questUnlocked(q,done)){done[q.id]=true;changed=true;}}
...
test('Ausgewählte neue Ergebnisse werden unabhängig vom Antwortprüfer nachgerechnet',()=>{
 const old=(id,i)=>questById(id).tasks.find(t=>t.key===id+':old:'+i);const val=(id,i)=>old(id,i).answer;
 assert.ok(equalDecimal(old('v0a',1).options[val('v0a',1)].split(' ')[0],(2+1/2).toString()));
