...
test('Offline-Installation enthält alle existierenden App-Dateien',async()=>{
 const h=serviceWorkerHarness();let task;h.events.install({waitUntil:p=>task=p});await task;assert.equal(h.stored.size,31);assert.ok(h.stored.has(h.scope+'js/learning-log.js'));assert.ok(h.stored.has(h.scope+'teacher-dashboard/shared/save-schema.js'));assert.ok(h.stored.has(h.scope+'js/visuals.js'));assert.ok(h.stored.has(h.scope+'js/visual-models.js'));assert.ok(h.skipped);
