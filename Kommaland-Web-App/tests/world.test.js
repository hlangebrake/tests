...
test('62 interaktive Objekte und endliche 3D-Geometrie',()=>{
 assert.equal(w.objects.length,62);assert.equal(w.objects.filter(o=>o.type==='quest').length,24);
...
test('Alle 62 interaktiven Ziele haben einen Weg vom Dorf aus',()=>{
 const failed=[];
...
test('Die Laufphysik erreicht alle Objekte ohne Hängenbleiben',()=>{
 ...
 for(const o of w.objects){w.teleport(0,30);reached=null;w.goToObject(o);let ticks=0;
  while(w.path.length&&ticks++<2600){time+=1/10;w.update(1/10,time,false);}
...
test('Bauabschluss enthüllt das echte neue Objekt und beendet den Aufbau genau einmal',()=>{
 const s=defaultState();w.sync(s);const o=w.objects.find(o=>o.id==='c6-v3');let count=0;
 assert.equal(o.after.visible,false);w.time=10;w.beginBuild('c6-v3',()=>count++);
...
test('Reduzierte Bewegung verkürzt den Aufbau ohne Verlust der Fertigstellung',()=>{
 ...
 w.beginBuild('c6-v1',()=>done=true);assert.equal(w.building.duration,.25);w.updateBuild(20.3);assert.ok(done);
});
...
test('Alle 22 gerichteten Wege stoppen die Figur an der richtigen geschlossenen Blockade',()=>{
 ...
 for(const g of w.gates){const r=REGIONS.find(r=>r.id===g.from);w.teleport(r.x,r.z);stopped=null;w.goToObject(w.objects.find(o=>o.id==='info-'+g.to));let ticks=0;while(w.path.length&&ticks++<2600)w.update(1/10,time+=1/10,false);assert.equal(stopped,g.id);}
});
...
test('Meisterkristalle sind vorher unsichtbar und nicht kollidierend; Erfolg verwandelt das Gebiet dauerhaft',()=>{
 const s=defaultState(),m=w.objects.find(o=>o.id==='c6-master-village');w.sync(s);assert.ok(m.hidden);assert.ok(!m.before.visible&&!m.after.visible);assert.equal(m.obstacle.active,false);
...
test('Die Abschlussanimation beginnt am sichtbaren Teilaufbau, nicht am zerstörten Anfangsmodell',()=>{
 const s=defaultState();s.progress['c6-v3']={step:8,ready:true,mistakes:0,hints:0};w.sync(s);const o=w.objects.find(o=>o.id==='c6-v3');assert.ok(o.working.visible);
 s.completed['c6-v3']={mistakes:0,hints:0,at:s.createdAt};delete s.progress['c6-v3'];w.sync(s);w.time=500;let calls=0;w.beginBuild('c6-v3',()=>calls++);
