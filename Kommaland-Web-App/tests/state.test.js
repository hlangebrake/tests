...
test('Version-1-Migration erhält Bauwerke, startet alte Teilaufgaben neu und öffnet neue Einstiegsquests',()=>{
 ...
 const clean=validateState(s);assert.equal(clean.schemaVersion,2);assert.deepEqual(clean.completed,s.completed);assert.deepEqual(clean.progress,{});assert.equal(clean.activeQuest,'v0a');assert.deepEqual(clean.migration,{from:1,restarted:1});assert.equal(clean.completed.v0a,undefined);
...
test('Fertige Antworten bleiben bis zum ausdrücklichen Bauabschluss als ready erhalten',()=>{
 const s=defaultState();s.progress.v0a={step:2,mistakes:0,hints:0,ready:true};
