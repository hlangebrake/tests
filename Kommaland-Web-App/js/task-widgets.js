...
export function widgetHTML(t,answer,solved,seed=1,activeField=0){
 ...
 if(t.type==='work'){
  ...
  return `<p class="question">${esc(t.text)}</p><p class="task-instruction">Bearbeite die ganze Rechnung. Tippe ein Feld an und nutze das Zahlenfeld. Erst „Antwort prüfen“ gibt die gesamte Rechnung ab.</p><div class="work-layout"><div class="work-scroll"><table class="work-calculation" aria-label="Rechnung mit ausfüllbaren Zwischenschritten"><tbody>${rows}</tbody></table></div><div class="work-pad"><div class="work-readout"><small id="workLabel">${esc(t.fields[activeField])}</small><output id="workValue">${esc(a[activeField]||'□')}</output></div><div class="keypad">${['7','8','9','4','5','6','1','2','3',',','0','⌫'].map(k=>`<button ${disabled} data-key="${k}" aria-label="${k==='⌫'?'Letzte Ziffer löschen':k}">${k}</button>`).join('')}</div><button ${disabled} class="secondary-button" id="workNext">Nächstes Feld</button></div></div>`;
 }
 if(t.type==='match'){
