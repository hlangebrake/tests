...
export function questIcon(q) {return ({fountain:'droplet',house:'home',crates:'bag',statue:'star',gate:'shield',stones:'route',lantern:'spark',tree:'leaf',boat:'anchor',tank:'droplet',clock:'clock',pavilion:'home',stall:'bag',banner:'flag',table:'bag',lift:'mountain',bridge:'route',mill:'wind',garden:'leaf',crystal:'gem',mushroom:'leaf',slime:'gem',boss:'shield',tower:'home',chest:'bag',dragon:'star',beacon:'spark'})[q.kind]||'star';}
export function numberLine(start=0,end=1,step=.1,selected=null) {
 const count=Math.round((end-start)/step); let ticks='';
 for(let i=0;i<=count;i++){const x=24+312*i/count,n=start+i*step;const label=n.toLocaleString('de-DE',{maximumFractionDigits:3});ticks+=`<path d="M${x} 31v${i%5===0?14:9}" stroke="currentColor"/><text x="${x}" y="64" text-anchor="middle" font-size="${count>12?10:12}" fill="currentColor">${esc(label)}</text>`;}
...
export function mathVisual(type, taskMode=false) {
 ...
 case 'inverse':s='<div class="visual-operation"><span>3 · 0,5</span>⇄<span>1,5 : 3</span></div>';cap='Multiplikation und Division sind Umkehroperationen.';break;
 ...
 case 'divide':s='<div class="visual-operation"><span>3 Liter</span>→<span>4 Becher</span></div>';cap='3 : 4 = 0,75 Liter je Becher';break;
...
export function lessonVisual(v){
 ...
 if(v.kind==='halves'){
  ...
  body=`<div class="fraction-explained"><div class="written-fraction"><b>${v.top}</b><b>${v.bottom}</b></div><div><p><b>oben:</b> ${v.top} Teil ist gemeint</p><p><b>unten:</b> ${v.bottom} gleich große Teile bilden das Ganze</p></div></div><div class="tenth-strip fraction-strip">${Array.from({length:v.bottom},(_,i)=>`<i class="${i<v.top?'filled':''}"></i>`).join('')}</div>`;
