...
export function validateState(raw) {
 if(!plain(raw)||raw.app!==APP_ID||![1,SCHEMA].includes(raw.schemaVersion)) throw new Error('Das ist kein unterstützter Kommaland-Spielstand (Version 1 oder 2).');
 ...
 for(const [id,value] of Object.entries(raw.progress)) {
  ...
  if(!legacy&&!s.completed[id]) {s.progress[id]={step:value.step,mistakes:value.mistakes,hints:value.hints};if(value.ready===true){if(value.step!==q.tasks.length-1)throw new Error('Ungültiger Bauabschluss.');s.progress[id].ready=true;}}
 }
 ...
 if(legacy) {
  s.activeQuest='v0a';
  s.migration={from:1,restarted:Object.keys(raw.progress).length};
 } else if(plain(raw.migration)&&raw.migration.from===1&&integer(raw.migration.restarted)) {
  s.migration={from:1,restarted:raw.migration.restarted};
 }
 return s;
}
...
export class SaveStore {
 ...
 load() {
  try {
   ...
   try{const parsed=parseSave(raw);this.migrated=JSON.parse(raw).schemaVersion===1;return parsed;}catch {
    this.rawBackup=raw;
