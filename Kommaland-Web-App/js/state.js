import {cleanQuestLevels,cleanTaskVariants} from '../teacher-dashboard/shared/difficulty.js';
...
import {validRoute, sourceLessons, pathKey, rememberOpenPaths} from './adventure.js';
...
export const defaultState = () => ({
 ...
 mastery:{}, competencies:{}, exams:{}, openPaths:{},
 activeQuest:'c6-v1', tutorial:false, encounters:{count:0,distance:0},
 settings:{sound:false,quality:'normal',largeText:false,reducedMotion:!!globalThis.matchMedia?.('(prefers-reduced-motion: reduce)').matches},
...
export function validateState(raw) {
 ...
 s.questLevels=cleanQuestLevels(raw.questLevels);
 ...
 for(const [id,value] of Object.entries(raw.progress)) {
  ...
  if(!legacy&&!s.completed[id]) {
   ...
   const variants=cleanTaskVariants(value.taskVariants,id);
   if(Object.keys(variants).length)s.progress[id].taskVariants=variants;
 ...
 // Additive 6.1 field; schema 3 and legacy filenames remain supported.
 if(raw.openPaths!==undefined){
  if(!plain(raw.openPaths)||Object.keys(raw.openPaths).length>11)throw new Error('Ungültige Wegfreigaben.');
  for(const [key,value] of Object.entries(raw.openPaths)){
   const [a,b,...rest]=key.split('~');
   if(rest.length||!validRoute(a,b)||key!==pathKey(a,b)||!plain(value)||!validDate(value.at))throw new Error('Ungültige dauerhafte Wegfreigabe.');
   s.openPaths[key]={at:value.at};
  }
 }
 // Reconstruct old eligible passages without certifying the destination region.
 rememberOpenPaths(s,validDate(raw.updatedAt)?raw.updatedAt:s.createdAt);
