...
export function checkAnswer(task, answer) {
  ...
  switch (task.type) {
    case 'number': case 'line': {
      if(task.accept){const x=decimal(answer),lo=decimal(task.accept.min),hi=decimal(task.accept.max);
       return !!x&&!!lo&&!!hi&&x.n*lo.d>lo.n*x.d&&x.n*hi.d<hi.n*x.d&&String(answer).split(/[,.]/)[1]?.length>=task.accept.minPlaces;}
      return equalDecimal(answer, task.answer);
    }
    case 'work': return Array.isArray(answer)&&answer.length===task.answer.length&&task.answer.every((x,i)=>equalDecimal(x,answer[i]));
...
export function describeAnswer(task) {
  if(task.type==='work')return task.fields.map((label,i)=>label+': '+String(task.answer[i]).replace('.',',')).join(' · ');
  if(task.accept)return 'Zum Beispiel '+task.answer+'; auch andere Zahlen im angegebenen offenen Intervall sind richtig.';
