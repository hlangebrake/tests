...
function model(c,s){
 ...
 case 'place':{
  const names=['Einer','Zehntel','Hundertstel','Tausendstel'],d=s.digits,den=10**(d.length-1);n=placeValue(d);
  art=`<div class="lab-place">${d.map((digit,i)=>`${i===1?'<span class="lab-comma">,</span>':''}<button type="button" class="lab-place-cell ${s.selected===i?'active':''}" data-lab-action="select-digit" data-value="${i}" aria-label="${names[i]}: ${digit}" aria-pressed="${s.selected===i}"><small>${names[i]}</small><b>${digit}</b><span>${fmt(digit/10**i)}</span></button>`).join('')}</div>`;
  const selected=s.selected;out=`${fixed(n/den,d.length-1)} = ${d.map((v,i)=>`${v} ${names[i]}`).join(' + ')}`;
  detail=`Die ausgewählte ${d[selected]} an der ${names[selected]}stelle ist ${fmt(d[selected]/10**selected)} wert. Eine 0 hält einen unbesetzten Platz frei.`;
  ...
  b+=bracket(40,70,480,`zehn ${names[selected]} = ${fmt(10/10**selected)}`);art+=image(b,'Zehn gleich große Teile des gewählten Stellenwerts.',112);
