 ...
 * lives here. No automatic-answer contents, keystrokes or device fingerprints. Version 7 stores only deliberately entered optional open-work notes (maximum 1200 characters), never grades their content.
...
export class LearningLog {
 ...
 async _attach(){
  ...
  if(this.getState().contentMigration&&!this.events.some(e=>e.type==='curriculum_changed'&&e.data.version===6))this.emit('curriculum_changed',{version:6,previousVersion:'5.1',credited:Object.keys(this.getState().completed).filter(id=>this.getState().completed[id].credit==='legacy'),started:Object.keys(this.getState().progress)},{});
 ...
 emit(type,data={},context=this.ctx){
  ...
  this.events.push(e);this.pending.set(e.eventId,e);this.current.updatedAt=e.timestamp;this.revision++;this.checkpoint();this.flush();for(const callback of this.listeners){try{callback(e);}catch(error){console.error("Regulationslogik:",error);}}return e;
 }
 startSession(){
 ...
 setScreen(screen){
  ...
  if(!['task','lesson','example','info','srl-scaffold'].includes(screen)){this.ctx={};}
 }
 ...
 help(type,screen,automatic=false,key=type,resumed=false){
  ...
  const h={type,screen,helpId:uid(),context:{...this.ctx},automatic};this.helps.set(key,h);this.emit(type+'_opened',{helpId:h.helpId,automatic,resumed,screen});
 }
 ...
 task(q,index,gameSession){
  if(gameSession?.practice)return; // Separate Version-7 event stream, never core progress.
  ...
  if(!this.events.some(e=>e.type==='task_started'&&e.taskRunId===gameSession.logTaskRunId))this.emit('task_started',{taskType:t.type,taskTitle:t.text,...(t.difficultyLevel?{difficultyLevel:t.difficultyLevel,variantId:t.variantId,slotId:t.slotId}:{})});
 }
 ...
 updateChallenges(){if(!this.studentId)return;const s=this.getState();for(const q of CATALOG.quests.filter(q=>q.kind==='challenge'&&q.active!==false))if(CATALOG.quests.filter(x=>x.kind==='regular'&&x.active!==false&&x.topicId===q.topicId).every(x=>s.completed[x.questId])&&!this.events.some(e=>e.type==='challenge_unlocked'&&e.questId===q.questId))this.emit('challenge_unlocked',{}, {questId:q.questId,topicId:q.topicId,kind:'challenge'});}
