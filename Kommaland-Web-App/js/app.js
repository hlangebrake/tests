...
function closePanel() {
 if(mode==='build'||mode==='gatebuild')return;
...
function showLocked(q) {
 ...
 $('lockedInfo').onclick=()=>showInfo(q.region);$('ignoreLocked').onclick=()=>showQuestIntro(q);
}
function showQuestIntro(q) {
 ...
 $('introInfo').onclick=()=>{returnFromInfo=()=>showQuestIntro(q);showKnowledge(q.tasks[progress?.step||0].lesson);};
 ...
 if(checkAnswer(t,s.answer)){
  ...
  if(!s.replay&&!s.encounter){
   if(s.index===s.q.tasks.length-1){
    state.progress[s.q.id]={step:s.index,mistakes:s.mistakes,hints:s.hints,ready:true};
...
function beginLearningStep(){
 const s=session,t=s.q.tasks[s.index],prev=s.q.tasks[s.index-1];
 if(s.encounter||s.q.challenge||(prev&&prev.group===t.group)){showTask();return;}
...
function finishQuest(){
 ...
 chooseNext(s.q.region);world.sync(state);persist();updateHUD();
...
function showKnowledge(id,automatic=false){
 ...
 $('knowledgeExample').onclick=()=>{returnFromInfo=()=>{returnFromInfo=back;showKnowledge(id,automatic);};showExample(id);};
}
...
function startGateTest(from,to){
 ...
 session={q,index:e.index,answer:null,solved:false,wrong:false,stepAttempts:0,mistakes:0,hints:0,replay:false,encounter:false,exam:true};returnFromInfo=null;persist();
...
function syncLabels(){
 for(const o of world.objects){const b=labels.get(o.id);if(!b)continue;
  ...
  b.classList.toggle('prerequisite',o.type==='gate'&&o.status?.sourcePassed);b.classList.toggle('done',!!o.done);b.classList.toggle('locked',o.type==='quest'&&!o.unlocked&&!o.done);
  ...
  b.querySelector('.label-text').textContent=o.type==='info'?'Wissen · '+regionById(o.region).name:o.type==='resident'?o.name+' · sprich mich an':o.type==='gate'?'Bereit für '+regionById(o.to).name+'?':o.q.title;
 }
}
