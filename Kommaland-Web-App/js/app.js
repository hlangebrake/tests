...
import {icon, questIcon, esc, mathVisual, numberLine, lessonVisual} from './ui.js';
...
function seals(){return REGIONS.filter(r=>r.id!=='castle'&&badgeEarned(r.id,state.completed)).length;}
...
function showLocked(q) {
 openPanel('locked',{title:'Ein Schritt nach dem anderen',tag:regionById(q.region).name,symbol:'lock',body:`<div class="intro-layout"><div class="story-medallion">${icon('lock')}</div><div class="story-text"><p>${esc(requirementText(q.id,state.completed))}</p><p>Diese Voraufgaben bereiten dich auf das neue Bauprojekt vor. Erkunden und am Infoschild nachlesen darfst du schon jetzt.</p></div></div>`,foot:secondary('Lernlandkarte','lockedMap','map')+primary('Wissen lesen','lockedInfo','book')});
...
function beginLearningStep(){
 returnFromInfo=()=>showTask();showKnowledge(session.q.tasks[session.index].lesson,true);
}
...
function showKnowledge(id,automatic=false){
 ...
 openPanel('lesson',{title:l.title,tag:automatic?'EIN GEDANKE VOR DER AUFGABE':'GENAU DAS WISSEN FÜR DIESEN SCHRITT',symbol:'book',className:'lesson-panel',body:`<div class="concept-layout"><p class="concept-text">${esc(l.text)}</p>${lessonVisual(l.visual)}</div>`,foot:secondary('Ein Beispiel lesen','knowledgeExample','eye')+primary(automatic?'Jetzt ausprobieren':session?'Zur Aufgabe':'Zurück','knowledgeDone')});
...
function showInfo(id,page=0) {
 ...
 openPanel('info',{title:l.title,tag:'WISSEN · '+r.name,symbol:'book',className:'lesson-panel',body:`<label class="knowledge-index" for="infoTopic">Einen Gedanken nachlesen<select id="infoTopic">${pages.map((p,i)=>`<option value="${i}" ${i===index?'selected':''}>${i+1}. ${esc(lessonById(p.id).title)}</option>`).join('')}</select></label><div class="concept-layout"><p class="concept-text">${esc(l.text)}</p>${lessonVisual(l.visual)}</div>`,foot:`<div class="info-controls">${index>0?secondary('Zurück','infoPrev'):''}${secondary('Ein Beispiel lesen','infoExample','eye')}<span class="pagination">${index+1} / ${pages.length}</span></div>`+primary(index<pages.length-1?'Weiter':'Zurück zur Welt','infoNext',index<pages.length-1?'arrow':'check')});
...
function showMap(selected=world.region().id) {
 ...
 const body=`<div class="map-body"><svg class="map-art landscape-map" viewBox="0 0 705 430" role="group" aria-label="Zusammenhängende Landschaft mit Küste, Wäldern, Bergzügen und acht Lernorten"><rect width="705" height="430" fill="#b6d4d0" rx="14"/><polygon points="${mapPolygon(OUTLINE)}" fill="#a4bd97" stroke="#e3dcb5" stroke-width="8"/>${clearings}${scenery}${lines}${nodes}<circle class="player-dot" cx="${p.x}" cy="${p.y}" r="5"/><text x="20" y="28" fill="#5f827c" font-size="10" letter-spacing="2">N ↑</text><text x="38" y="419" fill="#5f827c" font-size="10" letter-spacing="2">DIE SÜDKÜSTE</text></svg><div class="map-selection"><div><span class="section-tag">${icon(r.icon)}${esc(r.tag)}</span><h3>${esc(r.name)}</h3><p>${esc(r.topic)}</p><small>${esc(available?r.hint:requirementText(selected,state.completed))}</small></div><div class="map-quests">${questsIn(selected).map(q=>`<button data-map-quest="${q.id}" class="${state.completed[q.id]?'done':''}">${icon(state.completed[q.id]?'check':questUnlocked(q,state.completed)?questIcon(q):'lock')}<span>${esc(q.title)}${q.extension?'<em>Zusatzweg</em>':''}</span><small>${state.completed[q.id]||state.progress[q.id]?.ready?q.tasks.length:(state.progress[q.id]?.step||0)}/${q.tasks.length}</small>${icon('chevron')}</button>`).join('')}</div></div></div>`;
 openPanel('map',{title:'Deine Lernlandkarte',tag:`${seals()} VON 7 REISESIEGELN`,symbol:'map',subtitle:'Waldpfade, Lichtungen und Bergwege. Die Bauprojekte führen dich Schritt für Schritt weiter.',className:'map-panel',body,foot:`<span class="map-foot-note">Wähle einen Ort. Deine Figur läuft durch die Landschaft dorthin.</span>`+primary('Zum Infoschild','walkInfo','route')});
...
$('importFile').addEventListener('change',async e=>{
 ...
 try{
  ...
  confirmAction('Diesen Spielstand laden?',`${Object.keys(pendingImport.completed).length} Quests geschafft. Gespeichert am ${new Date(pendingImport.updatedAt).toLocaleString('de-DE')}. ${pendingImport.migration?'Die fertigen Bauwerke aus Version 1 bleiben erhalten. Alte angefangene Aufgaben starten neu; die neuen Einstiegsquests bleiben offen. ':''}Dein jetziger Stand wird ersetzt und als Rücksicherung behalten.`,'Spielstand laden',()=>{setState(store.replace(pendingImport));pendingImport=null;closeToWorld();toast('Dein Abenteuer ist wieder da.');});
...
function showWelcome(){
 openPanel('welcome',{title:'Willkommen in Kommaland.',tag:'DEIN DEZIMALZAHLEN-ABENTEUER',symbol:'spark',className:'welcome-panel',body:`<p class="welcome-intro">Im Funkeldorf ist das Sternenlicht erloschen. Mit kleinen Zahlen und klugen Ideen bringst du die Welt wieder zum Leuchten.</p><div class="welcome-features"><div class="welcome-feature">${icon('route')}<strong>Entdecken</strong><span>Ziehe den Steuerkreis oder tippe auf den Boden.</span></div><div class="welcome-feature">${icon('book')}<strong>Verstehen</strong><span>Jeder neue Gedanke beginnt mit einem Bild. Beispiele sind jederzeit da.</span></div><div class="welcome-feature">${icon('spark')}<strong>Verändern</strong><span>Löse kleine Quests. Die Welt wächst mit dir.</span></div></div><p class="small-note">Kein Zeitdruck. Keine verlorenen Leben. Du kannst alles noch einmal versuchen. Papier und Stift dürfen mitreisen.</p>`,foot:`<span>Dein Fortschritt wird automatisch lokal gespeichert.</span>`+primary('Abenteuer beginnen','welcomeStart')});
...
document.addEventListener('keydown',e=>{
 ...
 if(e.key==='Escape'){e.preventDefault();if(mode==='build')return;if(mode)closePanel();else showMenu();return;}
 if(mode){
...
requestAnimationFrame(()=>{
 ...
 if(store.migrated)toast('Deine fertigen Bauwerke wurden übernommen. Begonnene Aufgaben starten wegen der Überarbeitung neu; die neuen Einstiegsquests bleiben offen.');
...
if(new URLSearchParams(location.search).get('debug')==='1'){
 window.__kommaland={world,renderer,content:{REGIONS,QUESTS,INFO,LESSONS,questUnlocked},get state(){return state;},get session(){return session;},get mode(){return mode;},showKnowledge,showExample,showTask,interact,showMap,close:closeToWorld,persist,showQuestIntro,encounter};
}
