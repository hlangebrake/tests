...
function showTask() {
 ...
 $('taskHint').onclick=()=>{session.showHint=!session.showHint;if(session.showHint){recordHint();learning.help('hint','task');}else learning.closeHelp('hint');showTask();};
 if($('showSolution'))$('showSolution').onclick=()=>{session.showSolution=true;recordHint();learning.help('solutionStep','task');showTask();};
...
function showSuccess(){
 const s=session,final=s.q.id==='k4',areaComplete=!s.q.challenge&&allBasicDone(s.q.region,state.completed); 
...
function showResident(o){
 const qs=basicQuests(o.region),next=qs.find(q=>!state.completed[q.id]&&questUnlocked(q,state.completed))||qs.find(q=>!state.completed[q.id])||questById('master-'+o.region);
...
function closeProfileToWorld(){mode='profile-done';closeToWorld();}
...
async function exportSave(finish=false){
 if(!cleanName(state.learner?.studentName)){showProfile(()=>exportSave(finish));return;}
 try{
...
function showWelcome(){
 ...
 $('welcomeStart').onclick=()=>{state.tutorial=true;persist();closeToWorld();toast('Das Infoschild neben dir hilft dir beim Start. Beginne danach bei der kleinen Weglaterne.');};
}
...
$('tracker').onclick=()=>walkToObject(world.objects.find(o=>o.id==='info-'+(questById(state.activeQuest)||QUESTS[0]).region));
...
window.addEventListener('pageshow',e=>{if(e.persisted){learning.startSession();learning.resumeVisibility();}});
