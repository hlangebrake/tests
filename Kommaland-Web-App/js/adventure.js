/** Knowledge checks remain directional; physical path access is undirected.
 * Opening A→B never certifies B's skills, but permanently permits B→A.
 * This separates navigation permission from evidence about mathematical knowledge. */
...
export const pathKey=(a,b)=>[a,b].sort().join('~');
const directionReady=(state,from,to)=>!!state.mastery?.[from]&&PREREQUISITES[to].every(id=>state.competencies?.[id]);
/** Idempotent, additive migration. Only a fully eligible direction proves a path
 * was unlocked. visited[] alone cannot identify how a region was reached.
 * No mastery, competencies, quest statuses or learning events are invented. */
export function rememberOpenPaths(state,at=state.updatedAt||state.createdAt||new Date().toISOString()){
 state.openPaths??={};
 for(const [a,b] of PATHS){
  const key=pathKey(a,b);
  if(!state.openPaths[key]&&(directionReady(state,a,b)||directionReady(state,b,a)))state.openPaths[key]={at};
  if(state.openPaths[key]){delete state.exams?.[routeKey(a,b)];delete state.exams?.[routeKey(b,a)];}
 }
...
export function gateStatus(state,from,to){
 ...
 // The derived alternatives also cover a legacy in-memory state before validation.
 const pathOpen=!!state.openPaths?.[pathKey(from,to)]||directionReady(state,from,to)||directionReady(state,to,from);
 return {sourcePassed,missing,open:pathOpen,kind:pathOpen?'open':sourcePassed?'prerequisite':'source',unfinished:basicQuests(from).filter(q=>!state.completed?.[q.id]).length};
}
...
export function ensureExam(state,from,to){
 if(gateStatus(state,from,to).open)throw new Error('Dieser Weg ist bereits in beide Richtungen frei.');
...
export function finishExamRound(state,from,to){
 ...
 if(failed.length===0){
  ...
  // Commit physical access in BOTH directions, including any other paths that
  // became eligible through this proof. Opposite paused tests are now redundant.
  rememberOpenPaths(state,at);
